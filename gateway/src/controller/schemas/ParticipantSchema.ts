...
export type ParticipantIdSchema = Static<
  ReturnType<typeof ParticipantIdSchema>
>;
export function ParticipantIdSchema(options?: ObjectOptions) {
  return UUIDSchema({ description: "参加者ID", ...options });
}

export type ParticipantStateSchema = Static<
  ReturnType<typeof ParticipantStateSchema>
>;
export function ParticipantStateSchema(options?: ObjectOptions) {
  return Type.Union(
    ...
    {
      description:
        "参加者の状態 - pending: 参加待ち, ready: 準備完了, in_progress: 対戦中, battled: 対戦済み, eliminated: 敗退, champion: 優勝",
      ...options,
    }
  );
}
