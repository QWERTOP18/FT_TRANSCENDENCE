...
export function renderAdminScreen(appElement: HTMLElement, tournamentData: any, myUserId: string | null): void {
    ...
    if (tournamentData.state === 'reception') {
        actionButtonHTML = `<div class="mt-8 border-t border-gray-600 pt-6"><h3 class="text-lg font-semibold mb-2">トーナメントを開始する</h3><p class="text-sm text-gray-400 mb-4">参加者を確定し、対戦をオープンします。</p><button class="w-full px-4 py-2 metallic-button-green rounded text-white font-bold" onclick="window.router.openTournament()">トーナメントをオープンする</button></div>`;
    ...
    const contentHTML = `
        ...
        <div class="metallic-card p-8 rounded-lg text-white max-w-2xl mx-auto">
            ...
            <form onsubmit="window.router.handleAdminFormSubmit(event)">
                ...
                <button type="submit" class="w-full px-4 py-2 metallic-button-blue rounded text-white font-bold">変更を保存</button>
            </form>
            ${actionButtonHTML}
        </div>
    `;
