...
1. [PUT] \`tournaments/:id/participants/:id/ready\`で参加者をreadyにする。
1. [PUT] \`tournaments/:id/participants/:id/cancel\`で参加者をpendingにする。
1. [PUT] \`tournaments/:id/battle/start\`で参加者をin_progressにする。
1. [POST] \`tournaments/:id/battle/end\`で対戦結果を保存する。
1. トーナメントのステータスがcloseになるまで4, 5, 6, 7を繰り返す。closeになるとchampionが決定される。
